...
const SearchResults = ({ searchQuery, setSearchResVis }) => {
    ...
    return (
        ...
        onClick={e=>e.stopPropagation()}>
            <div className='users-results'>
                <h4>Users:</h4>
                <ul>
                    {usersResults.length ? usersResults.map(user => <li key={`user${user.id}`}><Link to={`/users/${user.id}`} onClick={e=>setSearchResVis(false)}>{formatResult(user.username)}</Link></li>) : <li>No users found</li>}      
                </ul>
            </div>
            <div className='songs-results'>
                <h4>Songs:</h4>
                <ul>
                    {songsResults.length ? songsResults.map(song => <li key={`song${song.id}`}><Link to={`/songs/${song.id}`} onClick={e=>setSearchResVis(false)}>{formatResult(song.title)}</Link></li>) : <li>No songs found</li>}      
                </ul>
            </div>
            <div className='playlists-results'>
                <h4>Playlists:</h4>
                <ul>
                    {playlistsResults.length ? playlistsResults.map(playlist => <li key={`playlist${playlist.id}`}><Link to={`/playlists/${playlist.id}`} onClick={e=>setSearchResVis(false)}>{formatResult(playlist.name)}</Link></li>) : <li>No playlists found</li>}      
                </ul>     
            </div>
        </div>
    )
